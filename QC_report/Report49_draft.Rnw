...
\title{Sequencing and mapping QC for library \Sexpr{libname}, COSMIC sample \Sexpr{cosmname}}
\date{\today}
...
#java -jar /data/software/picard/dist/picard.jar CollectAlignmentSummaryMetrics R=/data/genomes/hg19.bwa/hg19.fa I=/data/alignment_temp/Lib_1_bwamem_0.7.12.srt.bam O=picard_alignmentstats_lib1.txt
...
strCaption <- paste0("\\textbf{Sequencing and alignment statistics} Actuellement les statistiques ne sont pas correctes, car Sophia a omis les fragments qui ne s'alignaient pas de ce fichier.")
...
\caption{\textbf{Proportion of nucleotide per position} The present statistic is computed on the original files, without trimming. There are therefore potentially still adapters and all restriction sites are not trimmed.}
\end{center}
\end{wrapfigure}
...
clippedstartfwd=read.table(text=system(paste("samtools view -F 0x10",bamname,"|cut -f6|perl -ne \'{/^(\\d+)S/ && print \"$1\\n\"}\' |sort|uniq -c"),intern = T),header = F,as.is = TRUE)
clippedstartfwd=c(rep(clippedstartfwd$V2,clippedstartfwd$V1))
clippedstartrev=read.table(text=system(paste("samtools view -f 0x10",bamname,"|cut -f6|perl -ne \'{/^(\\d+)S/ && print \"$1\\n\"}\' |sort|uniq -c"),intern=T),header = F,as.is = TRUE)
clippedstartrev=c(rep(clippedstartrev$V2,clippedstartrev$V1))
clippedendfwd=read.table(text=system(paste("samtools view -F 0x10",bamname,"|cut -f6|perl -ne \'{/.*\\D(\\d+)S$/ && print \"$1\\n\"}\' |sort|uniq -c"),intern = T),header = F,as.is = TRUE)
clippedendfwd=c(rep(clippedendfwd$V2,clippedendfwd$V1))
clippedendrev=read.table(text=system(paste("samtools view -f 0x10",bamname,"|cut -f6|perl -ne \'{/.*\\D(\\d+)S$/ && print \"$1\\n\"}\' |sort|uniq -c"),intern = T),header = F,as.is = TRUE)
...
df=data.frame(cat=c("zero","one"),numbers=table[table$V1=="all",][1:2,3],percent=table[table$V1=="all",][1:2,5])
...
<<fig7plot>>
@
\caption{\textbf{Representation of k-mers per position, Read 1} We can see in pink some suspect over-represented homopolymers from base 30}
\end{center}
\end{wrapfigure}
...
<<fig8plot>>
@
\caption{\textbf{Representation of k-mers per position, Read 2} We can see in pink some suspect over-represented homopolymers from base 60}
\end{center}
\end{wrapfigure}
